...
static void* (*ikcp_malloc_hook)(void*, size_t) = NULL;
static void (*ikcp_free_hook)(void*, void *) = NULL;

// internal malloc
static void* ikcp_malloc(void* user2, size_t size) {
	if (ikcp_malloc_hook) 
		return ikcp_malloc_hook(user2, size);
	return malloc(size);
}

// internal free
static void ikcp_free(void* user2, void *ptr) {
	if (ikcp_free_hook) {
		ikcp_free_hook(user2, ptr);
	}	else {
		free(ptr);
	}
}
...
{
	return (IKCPSEG*)ikcp_malloc(kcp->user2, sizeof(IKCPSEG) + size);
}

// delete a segment
static void ikcp_segment_delete(ikcpcb *kcp, IKCPSEG *seg)
{
	ikcp_free(kcp->user2, seg);
}
...
void ikcp_qprint(const char *name, const struct IQUEUEHEAD *head)
{
	...
	for (p = head->next; p != head; p = p->next) {
		...
		printf("(%u %d)", seg->sn, (int)(seg->ts % 10000));
...
ikcpcb* ikcp_create(xx::Guid const* conv, void *user, void* user2)
{
	...
	kcp->buffer = (char*)ikcp_malloc(user2, (kcp->mtu + IKCP_OVERHEAD) * 3);
	if (kcp->buffer == NULL) {
		ikcp_free(user2, kcp);
		return NULL;
	}
...
void ikcp_release(ikcpcb *kcp)
{
	...
	if (kcp) {
		...
		if (kcp->buffer) {
			ikcp_free(kcp->user2, kcp->buffer);
		}
		if (kcp->acklist) {
			ikcp_free(kcp->user2, kcp->acklist);
		}
...
int ikcp_recv(ikcpcb *kcp, char *buffer, int len)
{
	...
	for (len = 0, p = kcp->rcv_queue.next; p != &kcp->rcv_queue; ) {
		...
		if (ikcp_canlog(kcp, IKCP_LOG_RECV)) {
			ikcp_log(kcp, IKCP_LOG_RECV, "recv sn=%u", seg->sn);
		}
...
static void ikcp_parse_fastack(ikcpcb *kcp, uint32_t sn)
{
	struct IQUEUEHEAD *p, *next;
...
static void ikcp_ack_push(ikcpcb *kcp, uint32_t sn, uint32_t ts)
{
	...
	if (newsize > kcp->ackblock) {
		...
		acklist = (uint32_t*)ikcp_malloc(kcp->user2, newblock * sizeof(uint32_t) * 2);
...
// input data
//---------------------------------------------------------------------
int ikcp_input(ikcpcb *kcp, const char *data, int size)
{
	uint32_t una = kcp->snd_una;
	uint32_t maxack = 0;
	int flag = 0;
	...
	if (data == NULL || size < (int)IKCP_OVERHEAD) return -1;
...
int ikcp_setmtu(ikcpcb *kcp, int mtu)
{
	...
	if (mtu < 50 || mtu < (int)IKCP_OVERHEAD) 
		return -1;
	buffer = (char*)ikcp_malloc(kcp->user2, (mtu + IKCP_OVERHEAD) * 3);
	if (buffer == NULL) 
		return -2;
	kcp->mtu = mtu;
	kcp->mss = kcp->mtu - IKCP_OVERHEAD;
