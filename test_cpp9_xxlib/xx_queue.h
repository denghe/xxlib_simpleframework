...
namespace xx
{
	...
	template <class T>
	Queue<T>::Queue(MemPool* mempool, size_t capacity = 0)
		: Object(mempool)
	{
		if (capacity < 8)
		{
		...
		auto bufByteLen = Round2n(capacity * sizeof(T) + sizeof(MemHeader)) - sizeof(MemHeader);
	...
	void Queue<T>::Reserve(size_t const& capacity, bool afterPush)
	{
		...
		auto newBufByteLen = Round2n(capacity * sizeof(T) + sizeof(MemHeader)) - sizeof(MemHeader);
