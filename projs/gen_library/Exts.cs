...
public static class GenExtensions
{
    ...
    /// 获取 C# 的默认值填充代码
    /// </summary>
    public static string _GetCSharpDefaultValueDecl(this object v)
    {
        if (v == null) return "";
        var t = v.GetType();
        if (t.IsValueType)
    ...
    public static string _GetCSharpTypeDecl(this Type t)
    {
        if (t._IsNullable())
        {
            return t.GenericTypeArguments[0]._GetCSharpTypeDecl() + "?";
        }
        if (t.IsArray)
        {
        ...
        else if (t._IsTuple())
        {
            ...
            for (int i = 0; i < t.GenericTypeArguments.Length; ++i)
            {
                if (i > 0)
                    rtv += ", ";
                rtv += _GetCSharpTypeDecl(t.GenericTypeArguments[i]);
            }
            rtv += ">";
            return rtv;
        }
        ...
        else
        {
            if (t.Namespace == nameof(TemplateLibrary))
            {
                if (t.Name == "List`1")
                {
                    return "List<" + _GetCSharpTypeDecl(t.GenericTypeArguments[0]) + ">";
                }
    ...
    /// 获取枚举对应的数字类型的类型名
    /// </summary>
    public static string _GetEnumUnderlyingTypeName(this Type e)
    {
...
" + sps + @"/// </summary>";
 ...
 + sps + @"// " + s;
    ...
    /// </summary>
    public static string _GetDesc<T>(this T t, int space) where T : ICustomAttributeProvider
    {
        foreach (var r_attribute in t.GetCustomAttributes(false))
        {
