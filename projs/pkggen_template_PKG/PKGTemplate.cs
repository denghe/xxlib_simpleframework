...
/* 
    ...
    Proxy 为数据代理, 具有 数据转发 / 汇聚 / 限速 / QOS / 基础安全校验 / 分流抗攻击 / 确保物理连接不断 等功用

    DB 为数据中心, 内存缓存热数据, 向内网提供各种 高效的 无阻塞的 逻辑单点的 数据操作.

    DBCommit 为数据同步提交服务, DB 的所有内存变化, 均及时生成相应的 数据库操作指令, 并放入 Redis 蓄水池
