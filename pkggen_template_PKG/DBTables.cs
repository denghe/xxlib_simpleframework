...
CREATE TABLE `log_game` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=44 DEFAULT CHARSET=utf8 COMMENT='玩家实时对局数据日志表(基表 )'
    */
    [Desc(@"玩家实时对局数据日志表(基表 )")]
    class log_game
    {
...
CREATE TABLE `log_login_out` (
  ...
  `create_time` bigint(20) NOT NULL COMMENT '创建时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=560 DEFAULT CHARSET=utf8 COMMENT='记录玩家上下线情况的日志. 分4种情况:   1. 上线     2. 断线     3. 重连.    4. 下线'
    */
